...
set CMDNAME=runPCsetup.cmd
set MAINPS=setup.ps1
::set time2=%time: =0%
...
NET SESSION > NUL 2>&1
IF %ERRORLEVEL% == 2 (
  echo %DATE% %TIME%:%CMDNAME%:ERROR:not Administrator mode. rc=%ERRORLEVEL%
  goto error-skip
)
powershell %STYLEOPTION% %PSHVER% -NoProfile -ExecutionPolicy Unrestricted "%~dp0%MAINPS%" %1 %2
if not %ERRORLEVEL% == 0 (
